...
def getAttributess(url):
    ...
    d={'Protocol':pd.Series(protocol),'Domain':pd.Series(domain),'Path':pd.Series(path),'Having_@_symbol':pd.Series(having_at_symbol),'Having_IP':pd.Series(having_ip),'Prefix_suffix_separation':pd.Series(prefix_suffix_separation),'Redirection_//_symbol':pd.Series(redirection_symbol),'Sub_domains':pd.Series(sub_domains),  'URL_Length':pd.Series(len_url),'age_domain':pd.Series(age_domain),'dns_record':pd.Series(dns_record),'domain_registration_length':pd.Series(domain_registration_length),'http_tokens':pd.Series(http_tokens),'statistical_report':pd.Series(statistical_report),'tiny_url':pd.Series(tiny_url),'web_traffic' : pd.Series(web_traffic) }

    data = pd.DataFrame(d)
    data = data.drop(columns=['Domain', 'Path', 'Protocol'], axis=1)
    return data


# prediction_input = getAttributess(
